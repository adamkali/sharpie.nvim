-- C# language handler for sharpie.nvim
-- Provides C#-specific symbol detection and type handling
...
-- Get the appropriate icon for a C# symbol, considering special cases
...
-- Parse return type from C# method detail
function M.get_return_type(symbol)
    ...
    -- Remove C# access modifiers and keywords
...
-- Get C#-specific indicators for a symbol (async, static, etc.)
function M.get_indicators(symbol)
    ...
    if detail:match("%s*async%s+") or M.returns_task(symbol) then
        table.insert(indicators, "")  -- Async indicator
    end

    -- Check for static indicator
    if detail:match("%s*static%s+") then
        table.insert(indicators, "")  -- Static indicator
    end
